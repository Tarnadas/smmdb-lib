...
const image3DS = (0, _symbol2.default)();
...
class Course {
  ...
  constructor(isWiiU, id, data, dataSub, path) {
    ...
    if (isWiiU) {
      ...
      this[image3DS] = new _tnl.Image(data.slice(0x2A05C, 0x2A05C + 0x157C0));
      this.changeEndian();
    }
  ...
  async to3DS() {
    ...
    const self = this;
    this.changeEndian();
    const data = this.gameStyle !== 0 ? this[courseData] : Buffer.concat([this[courseData].slice(0, 0xF0), ...(0, _from2.default)(function* () {
    ...
    const dataSub = this.gameStyle !== 0 ? this[courseDataSub] : Buffer.concat([this[courseDataSub].slice(0, 0xF0), ...(0, _from2.default)(function* () {
      for (let i = 0xF0; i < 0x145F0; i += 0x20) {
    ...
    let res = Buffer.concat([Buffer.from('00000000043004007D000000DDBAFECA3BA0B2B86E55298B00C01000', 'hex'), data, dataSub, Buffer.alloc(0x40), await new _tnl.Image(this.thumbnailPreview, this.thumbnail).to3DS(), Buffer.alloc(0x3800)]);
    res.writeUInt8(0x80, 0x2A023);
    let crc = (0, _bufferCrc2.default)(res.slice(0x2A020, 0x2A020 + 0x157C0 - 4)).readUInt32BE(0);
  ...
  async loadThumbnail() {
    ...
    if (!!this[image3DS]) {
      ...
      this[tnl] = await new _tnl.Jpeg(this.thumbnail).toTnl();
      this[tnlPreview] = await new _tnl.Jpeg(this.thumbnailPreview).toTnl();
    } else {
      try {
  ...
  async setThumbnail(thumbnail, isWide, doClip) {
    ...
    try {
      let jpeg = new _tnl.Jpeg(thumbnail);
      if (isWide) {
        this[tnl] = new _tnl.Tnl((await jpeg.toTnl(true, doClip)));
        this.thumbnail = await this[tnl].toJpeg();
        return this.thumbnail;
      } else {
        this[tnlPreview] = new _tnl.Tnl((await jpeg.toTnl(false, doClip)));
