...
async function loadCourse(coursePath, courseId) {
    ...
    return new _bluebird2.default((resolve, reject) => {
        fs.readFile(path.resolve(`${coursePath}/course_data.cdt`), async (err, data) => {
            ...
            let titleBuf = data.slice(_course.COURSE_CONSTANTS.COURSE_NAME_OFFSET, _course.COURSE_CONSTANTS.COURSE_NAME_OFFSET + _course.COURSE_CONSTANTS.COURSE_NAME_LENGTH);
            let title = "";
            for (let i = 0; i < _course.COURSE_CONSTANTS.COURSE_NAME_LENGTH; i += 2) {
            ...
            let makerBuf = data.slice(_course.COURSE_CONSTANTS.COURSE_MAKER_OFFSET, _course.COURSE_CONSTANTS.COURSE_MAKER_OFFSET + _course.COURSE_CONSTANTS.COURSE_MAKER_LENGTH);
            let maker = "";
            for (let i = 0; i < _course.COURSE_CONSTANTS.COURSE_MAKER_LENGTH; i += 2) {
                let charBuf = Buffer.allocUnsafe(2);
            ...
            let gameStyle = data.slice(_course.COURSE_CONSTANTS.COURSE_GAME_STYLE_OFFSET, _course.COURSE_CONSTANTS.COURSE_GAME_STYLE_OFFSET + 2).toString();
            let courseTheme = data.readUInt8(_course.COURSE_CONSTANTS.COURSE_THEME_OFFSET);
            try {
                let course = new _course2.default(courseId, data, dataSub, coursePath, title, maker, gameStyle, courseTheme);
                resolve(course);
            } catch (err) {
                reject(err);
            }
...
function loadCourseSync(coursePath, courseId) {
    ...
    let gameStyle = data.slice(_course.COURSE_CONSTANTS.COURSE_GAME_STYLE_OFFSET, _course.COURSE_CONSTANTS.COURSE_GAME_STYLE_OFFSET + 2).toString();
    let courseTheme = data.readUInt8(_course.COURSE_CONSTANTS.COURSE_THEME_OFFSET);
    return new _course2.default(courseId, data, dataSub, coursePath, title, maker, gameStyle, courseTheme);
}
